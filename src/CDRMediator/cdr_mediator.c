...
void *CDRMediatorThread(void *dt)
{
	...
	if(profile != NULL) {
		...
		LOG("CDRMediatorThread()","A CDRMediator thread into profile[%d]: %s is stoped",profile->cdr_server_id,profile->profile_name);
...
//#endif
{
	...
	if(cfg != NULL) {
		...
		if(mgr) {
			mgr->cdrm_interval = cfg->cdrm_interval;
			re5_mgr_mem_free(&re5_mgr_shm,0);
		}
	...
	if(opt_cli_mem.daemon_flag == 0) {
		for(i=0;i<cfg->profiles_number;i++) {
			if(profiles[i].cdr_active_flag == 't') {
				loop_flag = 't';
				break;
			} else loop_flag = 'f';
		}
	...
	LOG("CDRMediatorEngine()","PID: %d",getpid());
