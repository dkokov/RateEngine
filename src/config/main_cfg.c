...
main_cfg_t *main_cfg_init(void)
{
	...
	cfg = (main_cfg_t *)mem_alloc(sizeof(main_cfg_t));
...
void main_cfg_system_get(main_cfg_t *cfg)
{
	...
	while(params != NULL) {
		...
		if(strcmp(params->name,"DBVersion") == 0) {
			strcpy(cfg->system_db_version,params->value);
		}
...
void main_cfg_db_get(main_cfg_t *cfg)
{
	...
	sprintf(cfg->db,"host=%s dbname=%s user=%s password=%s ",
