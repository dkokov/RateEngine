...
my_cc_t *my_cc_parser(char *buffer)
{
	...
	my_request->cc_ptr = mycc_cc_api.cc_alloc();
...
int my_cc_mod_init(void)
{
	...
	if(func != NULL) {
		...
		ret = fptr(&mycc_cc_api);
		if(ret < 0) {
			LOG("my_cc_mod_init()","cc_bind_api(),ret: %d",ret);
			return RE_ERROR_N;
		}
	}
...
int my_cc_proto_bind(char *buf)
{
	...
	if(my != NULL) {
		...
		memcpy(buf,my->ack,strlen(my->ack));
