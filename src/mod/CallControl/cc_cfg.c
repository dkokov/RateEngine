	...
	if(cfg != NULL) {
		...
		while(params != NULL) {
			if(strcmp(params->name,"proto") == 0) {
				strcpy(cfg->proto,params->value);
			}
			
			if(strcmp(params->name,"CC-proto") == 0) {
				strcpy(cfg->cc_proto,params->value);
			}
			
			if(strcmp(params->name,"ip-version") == 0) {
				if(strcmp(params->value,"IPv4") == 0) cfg->ipv = ipv4;
				if(strcmp(params->value,"IPv6") == 0) cfg->ipv = ipv6;
			}						
