...
int rt_duckdb_rate_batch(rt_duckdb_t *ctx,db_t *pg_dbp,char leg,int limit)
{
	...
	snprintf(sql,sizeof(sql),
		...
		"    FROM ( "
		"      SELECT pt.cdr_id, pt.kind, cf.pos, cf.delta_time AS d, COALESCE(cf.fee,0) AS f, COALESCE(cf.iterations,0) AS it,"
		"             pt.sec AS cs, "
		...
		"           CASE WHEN it=0 OR u<=it THEN u ELSE it END AS charge "
		"    FROM ( "
		"      SELECT p.cdr_id, p.kind, cf.pos, cf.delta_time AS d, COALESCE(cf.fee,0) AS f, COALESCE(cf.iterations,0) AS it,"
