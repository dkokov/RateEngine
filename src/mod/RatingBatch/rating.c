...
{
	/* batch module: skip per-CDR save — handled by rt_batch_insert_ratings */
	if(rtp->pre->cdr_id > 0) {
		rtp->pre->rating_id = 1;  /* mark as "to be saved" */
	}
}
...
void rt_exec(db_t *dbp,racc_t *rtp,char leg)
{
	...
	/* batch module: skip per-CDR update_cdr and balance_exec
	   — handled by rt_batch_update_cdrs and rt_batch_update_balances */
}
...
void rt_main(db_t *dbp,rating_t *pre,char leg,int t)
{
	...
	/* save bacc/pcard info on pre before freeing rtp (needed for batch) */
	pre->bal_id = rtp->bacc_ptr->id;  /* reuse bal_id to carry bacc_id */
	if(rtp->bacc_ptr->pcard_ptr != NULL) pre->free_id = rtp->bacc_ptr->pcard_ptr->id;  /* reuse free_id to carry pcard_id */
...
/* Batch rating loop:
   1. Fetch CDRs
   2. Preload cache (subscribers, pcards)
   3. Rate all CDRs in memory (prefix match + calc_cprice)
   4. Bulk INSERT ratings (1 query)
   5. Bulk UPDATE cdrs leg_a (1 query)
   6. Bulk UPDATE balances (1 query per unique bacc)
*/
int rt_loop(rate_engine_t *rt_eng)
{
	...
	if(cdrs) {
		...
		results = (rt_rated_t *)mem_alloc_arr(pp,sizeof(rt_rated_t));
		if(results == NULL) {
			LOG("rt_loop()","cannot allocate results array");
			mem_free(cdrs);
			goto cleanup;
		}

		memset(results,0,pp * sizeof(rt_rated_t));

		/* PHASE 1: rate all CDRs in memory (subscriber lookup + prefix match + calc) */
		{
			int p;
			rating_t rt;
			rating_t *pre = &rt;
			...
			for(p = 0; p < pp; p++) {
				...
				/* find subscriber, rate, calc price — uses cache, minimal DB */
				rt_main(rt_eng->dbp,pre,rt_eng->leg,cdrs[p].cdr_rec_type_id);

				/* collect result */
				results[p].cdr_id = cdrs[p].id;
				results[p].cdr_rec_type_id = cdrs[p].cdr_rec_type_id;
				results[p].cprice = pre->cprice;
				results[p].billsec = pre->billsec;
				results[p].rate_id = pre->rate_id;
				results[p].bacc_id = pre->bal_id;   /* bacc_id stored here by rt_main */
				results[p].rating_mode_id = pre->rating_mode_id;
				results[p].tc_id = pre->tc_id;
				strncpy(results[p].timestamp,pre->timestamp,sizeof(results[p].timestamp)-1);
				results[p].free_billsec_id = pre->free_billsec_id;
				results[p].pcard_id = pre->free_id;  /* pcard_id stored here by rt_main */
				results[p].rating_id = 0;

				if(pre->rating_id > 0) {
					/* successfully rated — ready for batch save */
					results[p].status = 0;
					cc++;
				} else {
					results[p].status = -1;
		...
		/* PHASE 2: bulk INSERT ratings (1 query for all successful) */
		if(cc > 0) {
			int inserted = rt_batch_insert_ratings(rt_eng->dbp,results,pp);
			LOG("rt_loop()","batch INSERT: %d ratings",inserted);
		}

		/* PHASE 3: bulk UPDATE cdrs leg_a (1 query) */
		{
			int updated = rt_batch_update_cdrs(rt_eng->dbp,results,pp,rt_eng->leg);
			LOG("rt_loop()","batch UPDATE cdrs: %d",updated);
		}

		/* PHASE 4: bulk UPDATE balances */
		if(cc > 0) {
			rt_batch_update_balances(rt_eng->dbp,results,pp);
		}

		LOG("rt_loop()","batch done (leg %c), rated: %d/%d",rt_eng->leg,cc,pp);
...
cleanup:
	...
	if(log_debug_level >= LOG_LEVEL_INFO) {
		...
		if(pp > 0) {
			LOG("rt_loop()","batch times: %f total, %f avg/cdr, cdrs: %d",(t2-t1),((t2-t1)/pp),pp);
		}
	}
