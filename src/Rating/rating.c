...
void rating_loop(rate_engine *rt_eng)
{
	rating rt;
	rating *pre;
	struct timeval tim;
    ...
    if(cdrs) {
		...
		while(cdrs[pp].id > 0) {
			...
			copy_cdr_to_rating(&cdrs[pp],pre);	
			
			gettimeofday(&times, NULL);
			pre->start_timer = (((times.tv_sec)*1000000)+(times.tv_usec));
			
			rating_main(rt_eng->conn,pre,rt_eng->leg,cdrs[pp].cdr_rec_type_id);
			...
			(double)((pre->current_timer)-(pre->start_timer))/1000000,cdrs[pp].id);
