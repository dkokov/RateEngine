...
{
	int pid,cc_pid,rt_pid,cdrm_pid;
...
/*		case SIGABRT:
		...
		case SIGUSR1:
			if(pid == cc_pid) {
				LOG("signal_handler()","CallControl threads is stoping...PID:%d",cc_pid); 
				exit(EXIT_SUCCESS);
			} else if(pid == rt_pid) {
				LOG("signal_handler()","Rating threads is stoping...PID:%d",rt_pid);
				exit(EXIT_SUCCESS);
			} else if(pid == cdrm_pid) {
				LOG("signal_handler()","CDRM threads is stoping...PID:%d",cdrm_pid);
				exit(EXIT_SUCCESS);
			}
			break;
/*		case SIGUSR2:
