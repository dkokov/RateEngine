...
#ifdef MONIT
	#include "monit/smtp/send_email.h"
	#include "monit/snmp/snmp_defs.h"
	#include "monit/monit.h"
#endif
...
int chk_proc(int pid)
{
	...
	if (return_pid == -1) {
		DBG("chk_proc()","ERROR!PID:%d is not exist",pid);
	} else if (return_pid == 0) {
		DBG("chk_proc()","child [PID:%d] is still running",pid);
		return RE_SUCCESS;
	} else if (return_pid == pid) {
		DBG("chk_proc()","child [PID:%d] is finished",pid);
	}
...
re5_mgr_mem_t *re5_mgr_mem_init(shm_mem_t *shm,int mode)
{
	...
	if(mode == 1) {
		strcpy(shm->shm_name,RE5_MGR_SHMEM_NAME);
		shm->shm_fd = -1;
		shm->ptr = NULL;
		shm->shm_size = sizeof(re5_mgr_mem_t);
		shm->shm_mode = SHMEM_MODE_CREATE;
	} else if(mode == 2) {
		strcpy(shm->shm_name,RE5_MGR_SHMEM_NAME);
		shm->shm_fd = -1;
		shm->ptr = NULL;
...
int get_cdrs(void)
{
	...
	if(demonize == getpid()) {
		...
		if(new_pid == 0) {
			...
			ptr = re5_mgr_mem_init(&re5_mgr_shm,0);
...
{
	int pid,n,ret,fd,status;
	time_t res;
	char buf[16];
	
#ifdef MONIT
	monit_cfg_t *cfg_monit;
	monit_info_t info;
#endif
	...
	if(pid == ptr->re5_mgr_pid) {
...
#ifdef MONIT
		/* Monitor cfg */
		cfg_monit = monit_cfg_main(mcfg->cfg_filename);
		if(cfg_monit) {
			if(cfg_monit->t == unkw) {
				mem_free(cfg_monit);
				cfg_monit = NULL;
		...
		status = -1;
		wait(&status);
		DBG("re5_manager()","The child process finished with status %d.",status);
		
		ptr->re5_mgr_pid = 0;
		LOG("re5_manager()","'re5_manager' loop is stoped,PID: %d",pid);
	...
	if((ptr->re5_mgr_pid == 0)&&(ptr->cdrm_pid == 0)&&(ptr->cc_pid == 0)&&(ptr->rt_pid == 0)) {
		re5_mgr_mem_free(&re5_mgr_shm,1);
	
#ifdef MONIT
		if(cfg_monit != NULL) monit_cfg_free(cfg_monit);
#endif
