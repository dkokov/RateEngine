---
--- RateEngine PostgreSQL schema v2 (canonical for the API/docker/CI stack)
--- = rt_pgsql.sql + a trailing serial-sequence resync (see END).
---
...
(0,'no rounding','no rounding'),(1,'ceil','Round fractions up(0,[1-9] = 1)'),
...
---
--- v2 addition: resync every serial-id sequence to max(id).
--- Idempotent and safe on a FRESH load (empty tables -> sequence starts at 1),
--- and REPAIRS id-sequence drift from dump/restore migrations (e.g. the
--- time_condition_deff sequence) so INSERT ... RETURNING id never collides.
---
DO $$
DECLARE r record;
BEGIN
    ...
    LOOP
        EXECUTE format(
            ...
            || '(SELECT COALESCE(max(%I),0) FROM %I) > 0)',
