...
{
    global $RE;
    $conn = "host=".$RE['dbhost']." port=5432 dbname=".$RE['dbname']." user=".$RE['dbuser']." password=".$RE['dbpass']." ";
...
function re6_conn()
{
    ...
    $conn = "host=".$RE6['host']." port=5432 dbname=".$RE6['name']." user=".$RE6['user']." password=".$RE6['pass']." ";
...
function re6_query($query)
{
    global $RE6;
    return pg_query($RE6['dbconn'],$query);
}

function re6_cdr_storage_sched_ts()
{
	$ts = NULL;
		
	$sql = "select profi.profile_name,to_timestamp(sched.last_chk_ts),to_timestamp(sched.start_ts) 
			from cdr_storage_sched as sched,cdr_servers as cdr,cdr_profiles as profi 
			where sched.cdr_server_id = cdr.id and cdr.cdr_profiles_id = profi.id";
	
	$res = re5_query($sql);
	
	$i=0;
	while($row = pg_fetch_row($res)) {
		$ts[$i]['profile_name'] = $row[0];
		$ts[$i]['last']          = $row[1];
		$ts[$i]['start']         = $row[2];
...
function re5_racc_get_bacc_id($racc)
{
    $id = 0;
...
function re6_get_id_sms_cdr($cdr)
{
    $id = 0;
    
    $sql = "select id from cdrs where call_uid = '".$cdr['uuid']."' and start_ts = '".$cdr['ts']."';";
    $res = re6_query($sql);
...
function re5_get_sub_num($start,$end)
{
    $num = 0;
...
function re5_get_bal_num($start,$end)
{
    $num = 0;
...
function re5_get_cdrs($src,$start,$end)
{
    $arr = "";
...
function re5_bal_id_get_bacc($bal_id)
{
    $bacc = "";
...
function re5_bal_id_get_bacc_id($bal_id)
{
    $id = 0;
...
function re5_get_tariff_name($rate_id)
{
    $tariff_name = "";
/*    $sql = "SELECT tr.name,pr.comm 
            from rate as rt,tariff as tr,prefix as pr 
            where rt.tariff_id = tr.id and 
...
*/
    ...
    {
	$tariff_name = $row[0];
//	if(!empty($row[1])) $tariff_name .= "(".$row[1].")";
    }
    return $tariff_name;
}

function re5_get_rating_stat($prefix,$time1,$time2)
{
    $arr = "";
...
function re5_get_tariff_report($bid,$start,$end)
{
    $arr = "";
	...
	$arr[$c]['TariffName'] = re5_get_tariff_name($row[0]);
	...
	$free_billsec = re5_get_tid_free_billsec($row[0]);
	
	$arr[$c]['TariffName'] = re5_get_tariff_name($row[0])." (вкл.минути)";
	...
	$arr[$c]['TariffName'] = re5_get_tariff_name($row[0])." (НЕОГРАНИЧЕНИ)";
...
function re5_get_user_balances($bid,$status,$date)
{
    $balances = '';
    ...
    while($row = pg_fetch_row($res))
    {
	$tariff_report = re5_get_tariff_report($bid,$row[2],$row[3]);
	$balances[$i] = array(
				'UserBalanceID' => $row[0],
				'Amount'        => $row[1],
				'StartDate'     => $row[2],
...
function re5_bacc_id_get_balances($bid)
{
    $balances = '';
...
function re5_get_balance_id($bid)
{
    $bal_id = 0;
...
function re5_get_balance($bid)
{
    ...
    while($row = pg_fetch_row($res))
    {
	$tariff_report = re5_get_tariff_report($row[5],$row[2],$row[3]);
	$balances[$i] = array(
				'UserBalanceID' => $row[0],
				'Amount'        => $row[1],
				'StartDate'     => $row[2],
...
function re5_get_all_balances($end)
{
    $balances = '';
...
function re5_get_all_balances_v2($end,$bacc_like)
{
    $balances = '';
    $query = "SELECT bacc.username,bal.id,bal.amount,bal.start_date,bal.end_date,bal.active 
              from balance as bal,billing_account as bacc 
...
function re5_get_bal_last_updates($bal_id)
{
    $arr = "";
...
function re5_get_bill_file($bal_id)
{
    $arr = "";
...
function re5_get_rated_calls_report($balance_id)
{
    $arr = "";
...
function re5_get_rated_calls_report_2($balance_id)
{
    $arr = "";
...
function re5_get_operator_reports($bacc_id)
{
    $arr = '';
...
function re5_get_operator_traffic($report_id)
{
    $arr = '';
...
function re5_get_operator_traffic_brief($report_id)
{
    $arr = '';
	...
	$arr[$i]['count']   = $row[2];
	$arr[$i]['amount']  = $row[3];
	$arr[$i]['tariff']  = re5_get_tariff_name($arr[$i]['tariff_id']);
	$i++;
    }
    return $arr;
}

function re5_get_nsg_counters($cdr_server_id,$cur,$step,$var,$val)
{
    $arr = '';
...
function re5_get_rating_mode($id)
{
    $mode = '';
...
function re5_get_rating_mode_id($mode)
{
    $id = 0;
...
function re5_get_account_code_bid($bid)
{
    $acc = "";
...
function re5_get_billing_account_id($billing_account)
{
    $id = 0;
...
function re5_get_baccs($username)
{
    $bacc = "";
...
function re5_get_bacc_data($username)
{
    $arr = "";
...
function re5_get_bacc_data_v2($username)
{
    $arr = "";
...
function re5_get_bacc_data_2($bacc_id)
{
    $arr = "";
...
function re5_get_bacc_data_2_v2($bacc_id)
{
    $arr = "";
...
function re5_get_free_billsec($free_billsec_id)
{
    $sec = 0;
...
function re5_get_tid_free_billsec($tid)
{
    $sec = 0;
...
function re5_get_free_billsec_bal()
{
    $arr = "";
...
function re5_get_free_billsec_bal_id($bid)
{
    $id = 0;
...
function re5_get_calc_functions($tariff_id)
{
    $arr = "";
...
function re5_get_bplan_name($bid)
{
...
function re5_clg_get_bplan_id($clg)
{
    
}

function re5_get_bill_plans_tree_id($root_id)
{
    $arr = "";
...
function re5_get_rates_bplan_id($bplan_id)
{
    $arr = "";
...
function re5_get_bill_plans_2($bplan_id)
{
...
function re5_get_bill_plan_tree()
{
...
function re5_get_bill_plan_id($bill_plan)
{
    $id = 0;
...
function re5_insert_bill_plan($bplan)
{
    ...
    return re5_get_bill_plan_id($bplan['name']);
}

function re5_insert_billing_account($user)
{
    $username_test = $user['billing_account'];
    //hpvp_put_syslog(LOG_N,"[re5_insert_billing_account()],$username_test");
...
//    else 
    ...
    //hpvp_put_syslog(LOG_N,"[re5_insert_billing_account()];$query");
    re5_query($query);
}

function re5_insert_billing_account_2($user)
{
    $username_test = $user['username'];
    //hpvp_put_syslog(LOG_N,"[re5_insert_billing_account_2()],$username_test");
...
$dday = 0;
    ...
    //hpvp_put_syslog(LOG_N,"[re5_insert_billing_account_2()];$query");
    re5_query($query);

    return re5_get_billing_account_id($user['username']);
}

function re5_get_rating_account_id($rating_mode,$rating_account)
{
    $id = 0;
...
function re5_get_racc_bacc_id($rmode,$bacc_id)
{
    $racc = "";
...
function re5_get_racc($rmode,$racc_id)
{
    $racc = "";
...
function re5_get_racc_bplan_id($rmode,$racc_id)
{
    $id = 0;
...
function re5_get_racc_deff_id($rmode,$racc_id)
{
    $id = 0;
...
function re5_get_racc_acc($rmode,$racc_id)
{
    $acc = "";
    $query = "select ".$rmode." from ".$rmode." where id = ".$racc_id."";
    $res = re5_query($query);
...
function re5_get_racc_2($rmode,$racc)
{
    $arr = "";
...
function re5_get_racc_3($rmode,$bid)
{
    $arr = "";

    $query = "select racc.id,racc.".$rmode.",df.bill_plan_id 
	...
	$arr[$i]['bplan'] = re5_get_bplan_name($row[2]);
	$i++;
    }
    return $arr;
}

function re5_get_rmode_bplan($rmode,$racc_id)
{
    $name = "";
...
function re5_insert_rating_account($rating_mode,$rating_account,$billing_account_id,$bill_plan_id,$sm_bill_plan_id)
{
    ...
    $id = re5_get_rating_account_id($rating_mode,$rating_account);
    ...
    if($id) re5_query($query);
    else { 
	return 0;
    }
}
 
function re5_insert_rating_account_2($rating_mode,$rating_account,$billing_account_id,$bill_plan_id,$clg_nadi,$cld_nadi)
{  
    $query = "insert into ".$rating_mode." 
              (".$rating_mode.",billing_account_id) 
              values ('".$rating_account."',".$billing_account_id.")";
...
function re5_get_pcard_type_id($pcard_type)
{
    $id = 0;
    $query = "select id
              from pcard_type
...
function re5_get_pcard_status_id($pcard_status)
{
    $id = 0;
    $query = "select id
              from pcard_status
...
function re5_get_pcard_id($bacc,$type_id,$status_id,$amount,$start,$end)
{
    $id = 0;
    $query = "select id
              from pcard
...
function re5_get_pcard_id_2($bacc)
{
    $id = 0;
...
function re5_get_pcard_data($bacc_id)
{
    $arr = "";
    $query = "SELECT 
...
function re5_get_pcard_data_2($bacc_id)
{
    $arr = "";
    $query = "SELECT 
              pc.amount,ps.status,pt.name,pc.start_date,pc.end_date,pc.last_update,pc.id,pt.id,ps.id,pc.sim,
...
function re5_get_blocked_pcards()
{
    $arr = "";
...
function re5_insert_clg_history($clg,$bacc_id,$bplan_id,$operation)
{
...
function re5_insert_pcard($bacc_id,$pcard_tp,$pcard_sts,$amount,$start_date,$end_date)
{
...
function re5_insert_pcard_2($pcard)
{
    $query = "insert into pcard (billing_account_id,pcard_status_id,pcard_type_id,amount,start_date,end_date,last_update) 
...
function re6_insert_sms_cdr($cdr)
{
    $query = "insert into cdrs (cdr_server_id,cdr_rec_type_id,call_uid,start_ts,calling_number,called_number,dst,billsec,duration) 
              values (".$cdr['server'].",".$cdr['type'].",'".$cdr['uuid']."','".$cdr['ts']."','".$cdr['clg']."','".$cdr['cld']."','',0,0)";
    re6_query($query);

    return re6_get_id_sms_cdr($cdr);
}

function re5_delete_balances($bid)
{
    $sql = "delete from balance where billing_account_id = ".$bid.";";
    re5_query($sql);
}

function re5_delete_balance($id)
{
    $sql = "delete from balance where id = ".$id.";";
    re5_query($sql);
}

function re5_delete_free_balance($id)
{
    $sql = "delete from free_billsec_balance where id = ".$id.";";
    re5_query($sql);
}

function re5_delete_bacc($bid)
{
    $sql = "delete from billing_account where id = ".$bid.";";
    re5_query($sql);
}

function re5_delete_pcard($bid)
{
    $sql = "delete from pcard where billing_account_id = ".$bid.";";
    re5_query($sql);
}

function re5_delete_pcard_2($id)
{
    $sql = "delete from pcard where id = ".$id.";";
    re5_query($sql);
}

function re5_delete_racc($rmode,$racc_id)
{
    $sql = "delete from ".$rmode." where id = ".$racc_id.";";
    re5_query($sql);
}

function re5_delete_racc_bid($rmode,$bid)
{
    $sql = "delete from ".$rmode." where billing_account_id = ".$bid.";";
    re5_query($sql);
}

function re5_delete_racc_deff($rmode,$racc_id)
{
    $sql = "delete from ".$rmode."_deff where ".$rmode."_id=".$racc_id.";";
    re5_query($sql);
}

function re5_delete_rating($cdr_id)
{
    $sql = "delete from rating where call_id = ".$cdr_id."";
    re5_query($sql);
}

function re5_update_bplan($racc_id,$bplan_id)
{
    $sql = "update calling_number_deff set bill_plan_id = ".$bplan_id." where calling_number_id = ".$racc_id.";";
    re5_query($sql);
}

function re5_update_sm_bplan($racc_id,$bplan_id)
{
    $sql = "update calling_number_deff set sm_bill_plan_id = ".$bplan_id." where calling_number_id = ".$racc_id.";";
    re5_query($sql);
}

function re5_update_cdr($cdr_id)
{
    $sql = "update cdrs set leg_a = 0 where id = ".$cdr_id."";
    re5_query($sql);
}

function re5_update_bacc_days($bacc)
{
...
function re5_update_bacc_2($id,$usr,$bday)
{
    $sql = "update billing_account 
            set username = '".$usr."',
            billing_day = '".$bday."' 
...
function re5_update_bacc_username($id,$usr)
{
...
function re5_create_account($user)
{
    if(empty($user['rating_mode'])) return 0;

    $username_test = $user['billing_account'];
    ////hpvp_put_syslog(LOG_N,"[re5_create_account()],$username_test");

    // Insert BillingAccount
    $bacc_check_flag = 0;
    bacc_check:
    $billing_account_id = re5_get_billing_account_id($user['billing_account']);
    if(($billing_account_id == 0)&&($bacc_check_flag == 0))
    {
	re5_insert_billing_account($user);
	$bacc_check_flag = 1;
	goto bacc_check;
    }
    ...
    {
	////hpvp_put_syslog(LOG_N,"[re5_create_account()],A billing_account_id is null!!!");
	return 0;
    }

    $re5_bill_plan_id = re5_get_bill_plan_id($user['re5_bill_plan']);

    if($re5_bill_plan_id == 0)
    {
	////hpvp_put_syslog(LOG_N,"[re5_create_account()],A re5_bill_plan_id is null!!!");
	return 0;
    }

    if(!empty($user['sm_bill_plan'])) $sm_bill_plan_id = re5_get_bill_plan_id($user['sm_bill_plan']);
	else $sm_bill_plan_id = 0;
...
//    }
    ...
    $racc_check_flag = 0;
    racc_check:
    $rating_account_id = re5_get_rating_account_id($user['rating_mode'],$user['rating_account']);
    if(($rating_account_id == 0)&&($racc_check_flag == 0))
    {
	if(re5_get_rating_mode_id($user['rating_mode']) <= 4) 
	    re5_insert_rating_account($user['rating_mode'],$user['rating_account'],$billing_account_id,$re5_bill_plan_id,$sm_bill_plan_id);
	
	if(re5_get_rating_mode_id($user['rating_mode']) >= 5) 
	    re5_insert_rating_account_2($user['rating_mode'],$user['rating_account'],$billing_account_id,$re5_bill_plan_id,$clg_nadi,$cld_nadi);
    ...
    {
	////hpvp_put_syslog(LOG_N,"[re5_create_account()],A rating_account_id is null!!!");
	return 0;
    }
	...
	if(!empty($user['pcard_status'])) $pcard_status_id = re5_get_pcard_status_id($user['pcard_status']);
	else $pcard_status_id = 2;
	
	if(!empty($user['pcard_type'])) 
	{
	    $pcard_type_id = re5_get_pcard_type_id($user['pcard_type']);
	}
	else $pcard_type_id = 2;
	
	if($pcard_type_id) 
	{
	    if(($pcard_type_id == 2)AND(re5_get_pcard_id_2($billing_account_id))) goto end_func;
	
	    //echo "pcard_type_id = $pcard_type_id\n";
	    if((re5_get_pcard_id($billing_account_id,$pcard_type_id,$pcard_status_id,$user['amount'],$user['start_date'],$user['end_date'])) == 0)
	    {
		re5_insert_pcard($billing_account_id,$pcard_type_id,$pcard_status_id,$user['amount'],$user['start_date'],$user['end_date']);
	    }
	}
    }

    end_func:
    return array('re5_bacc_id' => $billing_account_id , 're5_racc_id' => $rating_account_id);
}
?>
