<?php
 ...
 function get_cdr_servers()
 {
    ...
    $query = "select id,server_name,switch_type_id,get_mode_id 
	...
	$servers[$i]['server_name'] = $row[1];
	$servers[$i]['switch_type_id'] = $row[2];
	$servers[$i]['get_mode_id'] = $row[3];
	$i++;
    }
    return $servers;
 }
...
/*	
 ...
 function get_free_billsec($process,$task)
 {
    global $dbconn;
    $free_billsec = 0;
    if($task->rate_id > 0 )
    {
	$query = "select free_billsec from free_billsec where tariff_id = ".$task->tariff_id."";
	$result = pg_query($dbconn,$query);
	while($row = pg_fetch_row($result))
	{
 ...
 function get_free_billsec_id($process,$task)
 {
    global $dbconn;
    $free_billsec = 0;
    $query = "select free_billsec from free_billsec where id = ".$task->free_billsec_id."";
    $result = pg_query($dbconn,$query);
    while($row = pg_fetch_row($result))
    {
	//$process->free_billsec = $row[0];
...
/* function get_norating_cdr($rating_mode_id)
 ...
 function insert_bill_plan($bill_plan,$bill_plan_type_id,$start_period,$end_period)
 {
    global $dbconn;
    $query = "insert into bill_plan 
              (name,bill_plan_type_id,start_period,end_period)
              values ('".$bill_plan."',".$bill_plan_type_id.",".$start_period.",".$end_period.")
             ";
    pg_query($dbconn,$query);
 }
 ...
 function insert_rating_account($rating_mode,$rating_account,$billing_account_id,$bill_plan_id)
 {
    ...
    { 
	echo "Don't have rating_account_id ...\n"; 
	exit;
    }
 }
