...
- A provisioning/admin user in the API auth store (`bin/re7-api-user.php add <user> --role admin`)
- `php` on PATH (used to parse JSON); `ab` or `hey` optional (better perf stats)

## Functional test
```bash
BASE=https://127.0.0.1:8443 API_USER=<user> API_PASS=<pass> \
  ./run_api_test.sh
...
BASE=... API_USER=<u> API_PASS=<p> ./api_perf.sh /ref/currencies 2000 32
BASE=... API_USER=<u> API_PASS=<p> ./api_perf.sh /bill-plans/SOMEPLAN 2000 32
```
