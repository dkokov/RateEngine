...
req POST /bill-plans "{\"name\":\"$PLAN\",\"type\":\"prepaid\"}";   assert_code 201 "create bill-plan"
req POST /bill-plans "{\"name\":\"$PLAN2\",\"type\":\"postpaid\"}"; assert_code 201 "create bill-plan2"
req POST /tariffs "{\"name\":\"$TAR\"}";                            assert_code 201 "create tariff"
req POST /prefixes "{\"prefix\":\"$PRE\"}";                         note "prefix $PRE (code $REPLY_CODE, may pre-exist)"
req POST /rates "{\"bill_plan\":\"$PLAN\",\"prefix\":\"$PRE\",\"tariff\":\"$TAR\"}"; assert_code 201 "create rate"
